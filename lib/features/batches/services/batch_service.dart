  ...
  Future<void> _backfillCanonicalFields({
    ...
    try {
      ...
      await db.runTransaction((tx) async {
        ...
        if (_s(m['tenantId']).isEmpty) {
          patch['tenantId'] = tenantId;
          need = true;
        }
        if (_s(m['storeId']).isEmpty) {
          patch['storeId'] = storeId;
          need = true;
        }
        if (_s(m['itemId']).isEmpty && _s(m['item_id']).isEmpty) {
          patch['itemId'] = itemId;
          need = true;
        }
        if (_s(m['itemType']).isEmpty && _s(m['item_type']).isEmpty) {
          patch['itemType'] = itemTypeName;
          need = true;
        }
        if (deliveryId != null &&
            deliveryId.isNotEmpty &&
            _s(m['deliveryId']).isEmpty) {
