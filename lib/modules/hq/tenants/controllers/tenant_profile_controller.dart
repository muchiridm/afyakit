import 'package:afyakit/modules/hq/tenants/providers/tenant_profiles_stream_provider.dart';
...
class TenantProfileController extends StateNotifier<TenantProfileEditState> {
  ...
  Future<bool> save({
    ...
    final svc = await _ref.read(tenantAdminServiceProvider.future);
  ...
  /// Delete a tenant profile by slug.
  ///
  /// Currently calls `deleteTenantProfile(slug)` which uses the backend
  /// default for soft/hard delete (controlled by the `hard` flag).
  Future<bool> delete(String slug) async {
    final svc = await _ref.read(tenantAdminServiceProvider.future);

    state = state.copyWith(busy: true, error: null);
    try {
      // hard delete: tell backend to purge
